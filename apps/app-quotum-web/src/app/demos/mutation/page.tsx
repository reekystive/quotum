    ...
    <div className="flex flex-col items-center gap-2 p-2">
      ...
      >
        <input
          type="text"
          autoComplete="off"
          className="w-40 rounded-sm border border-gray-500/50 px-2 py-1 text-base outline-none"
          value={name}
          onChange={(e) => setName(e.target.value)}
          placeholder="Magic happens"
        />
        <Button className="px-3" type="submit" disabled={!name.trim()}>
          Add User
        </Button>
      </form>
      ...
      <div className="flex min-w-[10rem] flex-col gap-1">
        ...
        {(userQuery.data ?? []).map((user) => (
          <UserItem
            ...
            pending={deleteUserMutationState.some(
